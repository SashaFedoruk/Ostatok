...
<div class="user-info col-lg-4 col-md-4 col-sm-6">
    ...
    <?php $form = ActiveForm::begin(); ?>
        ...
        <div id="section2">
            <?= $form->field($model, 'decoration_id')->dropDownList([]); ?>
        </div>
        <div id="section3" class="hidden">
            ...
            <div class="form-group">
                <?= Html::submitButton('Создать', ['class' => 'btn my2-btn']) ?>
            </div>
        </div>
    <?php ActiveForm::end(); ?>
...
$script = <<< JS
   ...
   //  $("*[name = 'AdsForm[width]']").change(updateData);
   // $("*[name = 'AdsForm[length]']").change(updateData);
   //  function updateData(){
   //      var width = $("*[name = 'AdsForm[width]']").val();
   //      var length = $("*[name = 'AdsForm[length]']").val();
   //      if(width == '' || length == ''){
   //          return;
   //      }
   //      var size = width * length / 1000000;
   //      var price = 0;
   //      $("#size").html(size);
   //      if(freeAds > 0){
   //          $("#price").html(price);
